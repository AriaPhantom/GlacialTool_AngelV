...
## Verification
- Run `cmd /c "set CI=1&& call build_all_versions.bat <nul"` after changes.
- Static sanity checks: no literal PowerShell escape artifacts (`` `r`n`` / `` `t``) and `git diff --check` clean.
