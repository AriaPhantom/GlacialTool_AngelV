...
#define SetDllPath _T("MVPACDlAd")
#else
//#define SetDllPath _T("SetDllPathA")
#define SetDllPath _T("odqOHwDr")
#endif

#define DM_REG_SRC _T("C:\\sptool\\SPReg.dll")
#define DM_SRC _T("C:\\sptool\\nj82119.dll")
...
BOOL CexampleApp::InitInstance()
{
	...
	HMODULE hmod = LoadLibrary(DM_REG_SRC);
	DWORD dwFun = (DWORD)GetProcAddress(hmod, SetDllPath);
	DWORD dwDmSrc = (DWORD)(TCHAR*)DM_SRC;
	_asm {
		push 0;
		push dwDmSrc;
		call dwFun;
	...
	// 收费注册
	CString dm_ver = g_dm->Ver();
	// long dm_ret = g_dm->RegEx(_T("jv965720b239b8396b1b7df8b768c919e86e10f"),_T("ldelyf366"),_T("123.129.231.44|123.129.231.45|123.129.231.85|123.129.231.86"));
	long dm_ret = g_dm->Reg(_T("nj8211961110befcdc7a94df9d22c65b31a4cb6"), _T("sp"));
	if (dm_ret != 1)
	{
		Log(_T("收费注册失败,返回值:%d"),dm_ret);
		MessageBox(NULL, _T("注册失败"), _T("错误"), MB_OK);
		delete g_dm;
		return 0;
	}

	Log(g_dm->Ver());
	//g_dm->DmGuardExtract("common","C:\\sptool\\Origin1.sys");

	// 这里可以做一些初始化设定
	// 比如开启保护盾,设置全局字库等
	
#if 1
	g_dm->DmGuardLoadCustom("common", "C:\\sptool\\SPSignedNew.sys");

    dm_ret = g_dm->DmGuard(1, _T("b2"));
	if (dm_ret != 1)
	{
		Log(_T("开启保护盾失败:%d"),dm_ret);
		MessageBox(NULL, _T("开启保护盾b2失败"), _T("错误"), MB_OK);
		delete g_dm;
		return 0;
	}

	dm_ret = g_dm->DmGuard(1, _T("hm nj82119.dll 1"));
	if (dm_ret != 1)
	{
		Log(_T("开启保护盾失败:%d"), dm_ret);
		MessageBox(NULL, _T("开启保护盾hm失败"), _T("错误"), MB_OK);
		delete g_dm;
		return 0;
	}

	dm_ret = g_dm->DmGuard(1, _T("f1"));
	if (dm_ret != 1)
	{
		Log(_T("开启保护盾失败:%d"), dm_ret);
		MessageBox(NULL, _T("开启保护盾f1失败"), _T("错误"), MB_OK);
		delete g_dm;
		return 0;
	}
#endif
